...
def recognize_plate(img, coords):
    ...
    for j in range(length):
        ...
        for i in range(width):
            ...
            if test[i,j] == 0:
                percentage += 1
        #print(percentage,"row",j)#
        if percentage > 0.6*width:
            #print("0.6 wa")
    ...
    # #     # perform another blur on character regionS
    #     roi = cv2.medianBlur(roi, 5)
    #     #cv2.imshow("roi", roi)
    #     #cv2.waitKey(0)
    #     try:
    #         print("trying")
    #         text = pytesseract.image_to_string(roi, lang = "tha",config='-c tessedit_char_whitelist=0123456789ภถคตจขชฎพฑธณรนยญบฐลฃฅฟฤหฆกฏดฌษศสวซงผปฉฮอทมฒฬฝฦ --psm 8 --oem 3')
    # #         # clean tesseract text by removing any unwanted blank spaces
    # #         #print("got this text "+text)
    ...
    for i in range(num_boxes):
        ...
        if class_name not in allowed_classes:
            ...
            if read_plate:
                ...
                #plate_number = helpPlate.plate(image,coor)
                if plate_number != None:
                   # print("bbbox plate number is not none",plate_number)
